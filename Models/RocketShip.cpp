...
RocketShip::RocketShip(string name, double maxFuelCapacity,
...
void RocketShip::setBoosterCount(int count) {
    if (count < 0) {
        cerr << "Error: Booster count cannot be negative.\n";
        return;
    }
    boosterCount = count;
}

string RocketShip::getType() const { return "Rocket Ship"; }
